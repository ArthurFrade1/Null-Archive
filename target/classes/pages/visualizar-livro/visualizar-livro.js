...
async function loadBookDetails() {
    ...
    try {
        const res = await fetch(`http://192.168.1.2:8081/book/info?id=${bookId}`, { credentials: "include" });
        book = await res.json();
        ...
        // Cover
        document.getElementById('book-cover').src = `http://192.168.1.2:8081/user/image/${book.storage_path_image}`;
        ...
        if (urlParams.has('admin')) {
            const adminRes = await fetch('http://192.168.1.2:8081/editor/data', { credentials: "include" });
...
async function lerArquivo(storage_path_file) {
    await fetch('http://192.168.1.2:8081/user/reading', {
...
async function moderarLivro(id, acao) {
    ...
    const response = await fetch('http://192.168.1.2:8081/admin/approve', {
        method: 'POST', credentials: "include",
        headers: { 'Content-Type': 'application/json' },
