  ...
  header.innerHTML = `
    ...
    <div id="logo" class="logo-tech" onclick="window.location.href='/home'">
    ...
    <nav class="user-nav">
        <div id="user-actions" class="nav-group" style="display:none;">
          ...
          <a href="../cadastro" class="nav-btn">Criar Conta</a>
        </div>
          
        <div id="account-actions" class="nav-group" style="display:none;">
          <span class="nav-text">@<span id="display-name"></span></span>
          <div id="admin-actions" style="display:none;">
              <a href="../curadoria" class="link-admin">Curadoria</a>
          </div>
          <a href="../publicar" class="btn-action">Novo Livro</a>
  ...
  document.getElementById("logout").addEventListener("click", async function (e) {
      ...
      const res = await fetch("http://127.0.0.1:80/editor/logout", {
        method: "POST",
        credentials: "include",
  ...
  async function loadData() {
    const res = await fetch("http://127.0.0.1:80/editor/data", {
      credentials: "include"
    });
