...
public class BookFileHandler implements HttpHandler {
    private final String baseDir = "C:\\Users\\arthu\\Desktop\\nullarchive_uploads\\books\\"; 
    ...
    public void handle(HttpExchange exchange) throws IOException {
        ...
        String action = "";
        String displayNames = "arquivo.pdf"; // Nome padrão caso algo falhe

        if (query != null) {
            // Uma forma simples de extrair os parâmetros
            for (String param : query.split("&")) {
                String[] pair = param.split("=");
                if (pair.length > 1) {
                    if (pair[0].equals("action")) action = pair[1];
                    if (pair[0].equals("name")) displayNames = java.net.URLDecoder.decode(pair[1], "UTF-8");
                }
            }
        }
        ...
        if("view".equals(action)){
            ...
            exchange.getResponseHeaders().set("Content-Disposition", "attachment; filename=\"" + displayNames + "\"");
        }else{
            HttpUtil.sendText(exchange, 400,"action invalido");
            return;
        }
